    ...
    <div id="poststuff" class="ep-box-row">
        <div class="ep-box-col-12">
                        ...
                        $booking_controller = EventM_Factory_Service::ep_get_instance( 'EventM_Booking_Controller_List' );
                         ...
                         echo sprintf('%d booking found',$bookings_data->posts_details->found_posts);
                         
                        ?>
                        <?php if(class_exists('EM_Advanced_Reports')){?>
                        <button type="button" id="ep_booking_export" class="button-primary ep-btn ep-ar-btn-primary">Export All</button>
                        
                        <?php } else {?>
                        <!--<button type="button" class="ep-disable-button button-primary ep-btn ep-ar-btn-primary ep-premium-locked">Export All <span><sup>PRO</sup></span></button>-->
                        <?php } ?>
                        ...
                        <div class="ep-reports-boooking-load-more">
                            ...
                            if( isset($bookings_data->posts_details->max_num_pages) && $bookings_data->posts_details->max_num_pages > 1) {?>
                                <div class="ep-report-load-more ep-frontend-loadmore ep-box-w-100 ep-my-4 ep-text-center">
                                    ...
                                    <button type="button" data-max="<?php echo $bookings_data->posts_details->max_num_pages;?>" id="ep-loadmore-report-bookings" class="ep-btn ep-btn-outline-primary"><span class="ep-spinner ep-spinner-border-sm ep-mr-1"></span><?php esc_html_e( 'Load more', 'eventprime-event-calendar-management' );?></button>
                                </div><?php
                            }?>
                        </div>
                        <?php } else{ echo esc_html('No Booking Found.');}?>
