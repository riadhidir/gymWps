...
class EventM_Booking_Admin_Meta_Boxes {
    ...
    */
    ...
    public function ep_tickets_attendies_box( $post ): void {
		include_once __DIR__ .'/views/meta-box-booking-attendies.php';
	}
	...
	public function ep_filter_booking_columns_content($column_name, $post_id){
        ...
        $booking = $booking_controller->load_booking_detail( $post_id );
        ...
        if($column_name == 'ep_title'){
            ...
            if( ! empty( $oldtitle ) ) {?>
                ...
                <strong>
                    <a class="row-title" href="<?php echo esc_url( get_edit_post_link() );?>">
                        <?php echo esc_html( $booking->event_data->em_name );?>
                        <?php do_action( 'ep_booking_list_content_after_event_title', $booking );?>
                    </a>
                </strong><?php
            }
        ...
        if ( $typenow == 'em_booking' ) {
            ...
            <select name="event_id" id="ep_event_id">
                ...
                <?php foreach( $events->posts as $event ) {?>
                    <option value="<?php echo esc_attr( $event->em_id ); ?>" <?php selected( $event->em_id, $selected_event ); ?>><?php echo esc_attr( $event->em_name ); ?></option>
                <?php } ?>
            </select>
            <input type="hidden" id="ep_booking_event_id" value="<?php echo !empty($selected_event) ? $selected_event : 'all';?>">
                <?php
            $payment_method = apply_filters('ep_payments_gateways_list', array());
            $selected_payment_method = '';
            if( isset( $_GET['payment_method'] ) ) {
                $selected_payment_method = $_GET['payment_method'];
            } ?>
            ...
            <input type="date" id="ep_booking_start_date" name="em_start_date" value="<?php echo isset($_GET['em_start_date']) ? $_GET['em_start_date'] : '';?>" placeholder="<?php esc_html_e( 'Start Date', 'eventprime-event-calendar-management' );?>"/>
            
            <span><?php esc_html_e( 'End Date', 'eventprime-event-calendar-management' );?></span>
            <input type="date" id="ep_booking_end_date" name="em_end_date" value="<?php echo isset($_GET['em_end_date']) ? $_GET['em_end_date'] : '';?>" placeholder="<?php esc_html_e( 'End Date', 'eventprime-event-calendar-management' );?>"/><?php 
        }
    }
    ...
    public function ep_booking_filters_argu($query ){
        ...
        if ( is_admin() && $pagenow=='edit.php' && $post_type == 'em_booking' && isset( $_GET['event_id'] ) && $_GET['event_id'] !='all' ) {
            ...
            $query->query_vars['meta_value'] = (int)$_GET['event_id'];
    ...
    public function ep_admin_notice_export(){
        if ( ! empty( $_REQUEST['ep_export_booking'] ) ) {
            ...
            printf('<div id="message" class="updated notice is-dismissable"><p>' . __('Exported %d bookings.', 'eventprime-event-calendar-management') . '</p></div>', $num_changed);
	}
    }
    /*
    ...
    public function ep_add_booking_export_btn(){
        ...
        ?>
            <script type="text/javascript">
                ...
                {
                    jQuery(jQuery(".wrap h1.wp-heading-inline")[0]).append("<a  id='ep_export_booking_all_btn' class='ep_export_booking_all_btn add-new-h2'>Export All</a>");
                });
            </script>
        <?php
    }
