...
class EventM_Admin_Menus {
    ...
    public function menus() {
        ...
        add_submenu_page( 'edit.php?post_type=em_event', esc_html__( 'Email Attendees', 'eventprime-event-calendar-management' ), esc_html__( 'Email Attendees', 'eventprime-event-calendar-management' ), 'manage_options', 'ep-bulk-emails', array( $this, 'bulk_emails_page' ) );
        ...
        add_submenu_page( "edit.php?post_type=em_event", esc_html__( 'Publish Shortcodes', 'eventprime-event-calendar-management'), esc_html__('Publish Shortcodes', 'eventprime-event-calendar-management'), "edit_posts", "ep-publish-shortcodes", array( $this, 'eventprime_publish_shortcodes' ) );
    ...
    public function ep_get_settings_tabs() {
        ...
        //$tabs['fes']                 = esc_html__( 'Frontend Submission', 'eventprime-event-calendar-management' );
    ...
    public function ep_emailer_list(){
        ...
        $emailers['event_submitted'] = array(
            ...
            'description'=>__('Informs the user on successfully submitting an event from the frontend form on this website.','eventprime-event-calendar-management'),
    ...
    public function ep_emailer_setting(){
        if(isset($_GET['section']) && isset($_GET['tab']) && sanitize_text_field($_GET['tab']) == 'emails'){
            $global_settings = EventM_Factory_Service::ep_get_instance( 'EventM_Admin_Model_Settings' );
            $global_options = $global_settings->ep_get_settings();
            $emailer_settings = array();
            
            ob_start();
            include __DIR__ .'/settings/emailers/emailer-'.sanitize_text_field($_GET['section']).'.php';
            $emailer_settings_content = ob_get_clean();
            $emailer_settings[$_GET['section']] = $emailer_settings_content;
            return apply_filters('ep_emailer_setting_add', $emailer_settings);
        }
    }
    ...
    public function ep_checkout_field_types() {
        ...
        return apply_filters( 'ep_checkout_fields_add', $field_types );
    }
    ...
    public function ep_setting_submit_button_callback(){ 
        $tabs_list = array('forms','extensions','checkoutfields');
    ...
    public function get_label_section_lists() {
        $labelsections = array( 'Event-Type', 'Event-Types', 'Venue', 'Venues', 'Performer', 'Performers', 'Organizer', 'Organizers' );
    ...
    public function ep_get_settings_front_views_content( $active_sub_tab ) {
        ...
        $settings = EventM_Factory_Service::ep_get_instance( 'EventM_Admin_Model_Settings' );
        $options['global'] = $settings->ep_get_settings();
        $core_tabs = array('events','eventtypes','performers','venues','organizers');
