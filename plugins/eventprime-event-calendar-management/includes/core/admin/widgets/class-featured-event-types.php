            ...
            $html = '<div class="widget widget_featured_events ep-widgets"><div class="widget-content">';
                ...
                if( ! empty( $types->terms ) ){
                    ...
                    foreach ( $types->terms as $type ) {
                        $html .= '<div class="ep-featured-event-type ep-widget-block-wrap ep-d-flex ep-p-2 ep-my-3 ep-shadow-sm ep-border ep-rounded-1">';
                            $title = $type->name;
                            ...
                            $html .= '</div>';
                            $html .= '<div class="ep-fdata"><div class="ep-fname ep-mt-2"><a href="'.esc_url( $type->event_type_url ).'">'.esc_attr( $type->name ).'</a></div>';              
                            $html .= '</div>';
                        $html .= '</div>';
                    }
                }else{
...
function em_load_featured_type() {
    register_widget('eventm_featured_type');
}

add_action('widgets_init', 'em_load_featured_type');
