        ...
        <tr valign="top" >
            ...
            <td class="forminp forminp-text">
                <select name="frontend_submission_roles[]" id="frontend_submission_roles" multiple="multiple" class="ep-form-control">
                    <?php foreach( ep_get_all_user_roles() as $key => $role ){ echo $key;?>
                        <option value="<?php echo esc_attr( $key );?>" <?php if( in_array( $key , $options['global']->frontend_submission_roles, true ) ) { echo 'selected="selected"'; } ?>>
                            <?php echo $role;?>
                        </option><?php
                    }?>
                </select>
