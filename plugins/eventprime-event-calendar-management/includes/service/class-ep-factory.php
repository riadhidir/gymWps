...
class EventM_Factory_Service {
     ...
     */
    public static function ep_get_event_types( $fields ) {
		...
		if( !empty( $event_type ) && !empty( $fields ) ) {
			$event_type_data = $event_type->get_event_type_field_data( $fields );
		}
	 ...
	 */
	public static function ep_get_child_events( $post_id ) {
		$posts = get_posts(
			array(
				'post_parent' => $post_id,
				'post_type'   => 'em_event',
			...
			if( ! empty( $ticket_qty ) ) {
				...
				if( ! empty( $bookings['bookings'] ) && count( $bookings['bookings'] ) > 0 ) {
					foreach( $bookings['bookings'] as $booking ) {
						if( isset( $booking->em_order_info ) && isset( $booking->em_order_info['tickets'] ) && ! empty( $booking->em_order_info['tickets'] ) ) {
