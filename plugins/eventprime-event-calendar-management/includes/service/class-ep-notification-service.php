...
class EventM_Notification_Service {
    ...
    public static function booking_confirmed( $booking_id ) {
        ...
        $mail_body = isset($global_setting->booking_confirmed_email) ? $global_setting->booking_confirmed_email : '';
        $subject = isset($global_setting->booking_confirm_email_subject) ? $global_setting->booking_confirm_email_subject : '';
        self::configure_mail();
        $to = '';
        $booking_user_email = $booking_user_name = '';
        ...
        if(isset($global_setting->send_booking_confirm_email) && !empty($global_setting->send_booking_confirm_email)){
            ...
            if(isset($booking->event_data->em_start_date)){
                ...
                if( ! empty( $booking->event_data->em_start_time ) ) {
                    $event_date_time .= ', ' . esc_html( $booking->event_data->em_start_time );
                }
                    ...
                    $attendee_name_html .= '<tr><th colspan="2" style="text-align:left;border-top-width:4px;color:#737373;border:1px solid #e4e4e4;padding:12px">'.esc_html__( 'Attendees '.$i, 'eventprime-event-calendar-management' ).' </th>';
                        $attendee_name_html .= '<td colspan ="2" style="text-align:left;border-top-width:4px;color:#737373;border:1px solid #e4e4e4;padding:12px">';
                            ...
                            foreach( $booking_attendees_field_labels as $labels ){
                                ...
                                $attendee_name_html .= '<span>'. $labels .' : '. $at_val .' </span><br/>';
                            }
        ...
        */
        ...
        $subject = isset($global_setting->admin_booking_confirmed_email_subject) ? $global_setting->admin_booking_confirmed_email_subject : __('Booking Confirmation','eventprime-event-calendar-management');
        $mail_body = str_replace("(user_email)", $booking_user_email, $mail_body);
        $mail_body = str_replace("(event_name)", $booking->em_name, $mail_body);
        $mail_body = str_replace("(event_date)", date('Y-m-d h:i a',$booking->em_date), $mail_body);
        ...
        $mail_body = str_replace("(booking_date)", date('Y-m-d h:i a',$event_date), $mail_body);
    ...
    public static function booking_refund( $booking_id ) {
        ...
        $settings = EventM_Factory_Service::ep_get_instance( 'EventM_Admin_Model_Settings' );
        $global_setting = $settings->ep_get_settings();
        $mail_body = isset($global_setting->booking_refund_email) ? $global_setting->booking_refund_email : '';
        $subject = isset($global_setting->booking_refund_email_subject) ? $global_setting->booking_refund_email_subject : __('Booking Refund','eventprime-event-calendar-management');
        self::configure_mail();
        $to = '';
        $booking_user_email = $booking_user_name = '';
        $user_id = isset($booking->em_user) ? (int) $booking->em_user : 0;
        ...
        */
        ...
        $to = $admin_email; 
        $subject = sprintf(__('Booking Refund on Booking ID# %d','eventprime-event-calendar-management'),$booking_id);        
        $body = sprintf(__('A refund of %s has been issued to booking #%d for %s','eventprime-event-calendar-management'),ep_price_with_position($ticket_sub_total),$booking_id,$booking->em_name);
        if(empty( ep_get_global_settings( 'disable_admin_email' ) ) ) {
    ...
    public static function booking_pending( $booking_id ) {
        ...
        $subject = isset($global_setting->booking_pending_email_subject) ? $global_setting->booking_pending_email_subject : __('Booking Pending','eventprime-event-calendar-management');
        self::configure_mail();
        $to = '';
        $booking_user_email = $booking_user_name = '';
        $user_id = isset($booking->em_user) ? (int) $booking->em_user : 0;
        ...
        */
        ...
        $body = sprintf(__('User %s has Booking Pending with Booking ID #%d.','eventprime-event-calendar-management'),$booking_user_email,$booking_id);
    ...
    public static function booking_cancel( $booking_id ) {
        ...
        $subject = isset($global_setting->booking_cancelation_email_subject) ? $global_setting->booking_cancelation_email_subject : __('Booking Cancellation','eventprime-event-calendar-management');
        self::configure_mail();
        $to = '';
        $booking_user_email = $booking_user_name = '';
        $user_id = isset($booking->em_user) ? (int) $booking->em_user : 0;
    ...
    public static function user_registration($user_data=null ) { 
        ...
        if(isset($user_data->password)){
            $mail_body .= "Your auto generated password is ".$user_data->password;
        }
        $registration_email_subject= $global_setting->registration_email_subject;
        
        if(empty($registration_email_subject)){
            $registration_email_subject= __('Successfully Registered.','eventprime-event-calendar-management');
        }
    ...
    {  
        ...
        if(empty($global_setting->send_event_submitted_email))
           return;
    ...
    {  
        ...
        if(empty($global_setting->send_event_approved_email))
           return;
        ...
        $subject = isset($global_setting->event_approved_email_subject) ? $global_setting->event_approved_email_subject : __('Event Published','eventprime-event-calendar-management');
        if(empty($global_setting->disable_frontend_email)){
            wp_mail($to, $subject, $mail_body);
        }
    }
    ...
    private static function gcal_link($event, $venue) {
        $html = '<div id="authorize-button" class="kf-event-add-calendar em_color dbfl">
            <a class="em-events-gcal em-events-button em-color em-bg-color-hover em-border-color" href="https://www.google.com/calendar/event?action=TEMPLATE&text='.urlencode($event->em_name).'&dates='.gmdate('Ymd\\THi00', ($event->em_start_date)).'/'.gmdate('Ymd\\THi00', ($event->em_end_date)).'&details='.urlencode($event->description).$venue->name.'" target="_blank" title="'.esc_html__('Add To Google Calendar', 'eventprime-event-calendar-management').'">';
