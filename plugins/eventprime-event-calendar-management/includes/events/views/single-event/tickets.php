...
<div class="ep-box-col-4" id="right-area position-relative">
    <?php do_action( 'ep_event_detail_before_ticket_block', $args->event );?>
    <div class="ep-box-row ep-border ep-rounded">
        <?php 
        if( ! empty( $args->event->em_enable_booking ) && $args->event->em_enable_booking != 'bookings_off' ) {
            if( $args->event->post_status == 'publish' || ( ! empty( $all_tickets ) && count( $all_tickets ) > 0 ) ) {
                if( ! check_event_has_expired( $args->event ) ) {?>
                    <div class="ep-box-col-12 ep-pl-3 ep-pt-3">
                ...
                <div class="ep-box-col-12 ep-p-4">
                                ...
                                if( isset( $args->event->ticket_price_range['multiple'] ) && $args->event->ticket_price_range['multiple'] == 1 ) {
                                    if( $args->event->ticket_price_range['min'] == $args->event->ticket_price_range['max'] ) {?>
                                        <h6 class="ep-fs-6"><?php
                                            if( $args->event->ticket_price_range['min'] ){
                                                esc_html( ep_price_with_position( $args->event->ticket_price_range['min'] ) );
                                            } else{
                                    ...
                                    <h6 class="ep-fs-6"><?php
                                        if( $args->event->ticket_price_range['price'] ){
                                            esc_html( ep_price_with_position( $args->event->ticket_price_range['price'] ) );
                                        } else{
                    ...
                    <div class="ep-mt-3 d-grid gap-2 d-md-block" id="ep_single_event_ticket_now_wrapper"><?php 
                        if( $args->event->em_enable_booking == 'external_bookings' ) {
                            ...
                            <button type="button" class="ep-btn ep-btn-light ep-text-danger ep-box-w-100 ep-mb-2 ep-py-2">
                            ...
                            if( ! empty( $args->event->ticket_price_range ) ) {?>
                                <button type="button" id="ep_single_event_ticket_now_btn" class="ep-btn ep-btn-dark ep-box-w-100 ep-mb-2 ep-py-2" ep-modal-open="ep_single_event_page_ticket_modal">
                                    <?php echo esc_html( $get_ticket_now_text ); ?>
                                </button><?php
                            }
                        }?>
                    ...
                    <div class="ep-modal-body"> 
                        <div class="ep-box-row">
                            ...
                            if( $em_event_seating_type == 'seats' ) {
                                ...
                                <div class="ep-box-col-8 ep-py-3" id="ep_event_ticket_modal_left">
                                        ...
                                        if( ! empty( $check_ticket_visibility['status'] ) ){
                                            ...
                                            <div class="ep-box-row ep-mb-3 ep-box-w-100 ep-border-bottom <?php echo esc_attr( $ticket_disabled_class );?>" id="ep_single_modal_ticket_<?php echo absint( $ticket->id );?>" data-ticket_id="<?php echo absint( $ticket->id );?>" data-ticket_data='<?php echo json_encode( $ticket );?>'>
                                                <div class="ep-box-col-12 ep-fs-5 ep-fw-bold ep-d-flex ep-align-items-center"> 
                                                    ...
                                                    echo esc_html( $ticket->name );?> </span>
                                                </div>
                                                            ...
                                                            <div class="ep-text-small ep-text-white ep-mt-2">
                                                                <span class="ep-bg-danger ep-py-1 ep-px-2 ep-rounded-1 ep-text-smalll">
                                                                    ...
                                                                    if( $ticket_sold_out == 1 ) {
                                                                        ...
                                                                        esc_html_e( 'tickets left!', 'eventprime-event-calendar-management' );
                                                                    }?>
                                                                </span>
                                                            </div><?php
                                                        }?>
                                                ...
                                                <div class="ep-box-col-12 ep-text-small ep-mt-2 ep-text-muted">
                                                    ...
                                                    if( absint( $ticket->min_ticket_no ) > 0 ) {?>
                                                        <div class="ep-text-small ep-d-inline-flex me-3">
                                                            <span class="material-icons-outlined ep-fs-6 ep-align-top">task_alt</span> <?php esc_html_e( 'Min Qnty', 'eventprime-event-calendar-management' );?>: <?php echo absint( $ticket->min_ticket_no );?>
                                                        </div><?php
                                                    }
                                                    if( absint( $ticket->max_ticket_no ) > 0 ) {?>
                                                        <div class="ep-text-small ep-d-inline-flex">
                                                            <span class="material-icons-outlined ep-fs-6 ep-align-top">task_alt</span> <?php esc_html_e( 'Max Qnty', 'eventprime-event-calendar-management' );?>: <?php echo absint( $ticket->max_ticket_no );?>
                                                        </div><?php
                                                    }?>
                                                </div>
                                                <?php
                                                    ...
                                                    <div class="ep-ticket-offers-wrapper ep-box-col-12 ep-d-flex ep-text-small" >
                                                        ...
                                                        foreach( $event_offers['show_ticket_offers'][$ticket->id] as $offer ) {?>
                                                            <div class="ep-text-small ep-d-inline-flex ep-border ep-rounded ep-p-2 ep-flex-column ep-mr-2 ep-box-w-25 ep-position-relative <?php if(! empty( $offer->em_ticket_offer_type ) && $offer->em_ticket_offer_type == 'volume_based'){ echo esc_html( 'em_ticket_volumn_based_offer' );}?>" data-offer_data='<?php echo json_encode( $offer );?>'>
                                                                ...
                                                                $offer_date = EventM_Factory_Service::get_offer_date( $offer, $args->event );
                                                                if( ! empty( $offer_date ) ) {?>
                                                                    <div class="ep-text-small ep-text-muted ep-mt-1">
                                                                        <span class="material-icons-outlined ep-fs-6 ep-text-small ep-align-top">schedule</span> <?php echo esc_html( $offer_date );?>
                                                                    </div><?php
                                                                }
                                ...
                                <div class="ep-box-col-4" id="ep_event_ticket_modal_right">
                                    <?php 
                                    $total_price = 0;
