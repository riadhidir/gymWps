<div class="ep-box-row">
    <div class="ep-box-col-12" id="ep_single_event_image">
        <div class="ep-single-event-slide-container">
            <?php $event_gallery = ( ! empty( $args->event->em_gallery_image_ids ) ? $args->event->em_gallery_image_ids : '' );
            if( empty( $event_gallery ) ) {?>
                <img src="<?php echo esc_url( $args->event->image_url );?>" alt="<?php echo esc_attr( $args->event->name ); ?>" class="ep-d-block ep-box-w-100" /><?php
            } else{?>
                <ul class="ep-rslides ep-m-0 ep-p-0" id="ep_single_event_image_gallery">
                    ...
                    if( ! empty( $args->event->image_url ) && ! empty( has_post_thumbnail( $args->event->em_id ) ) ) {?>
                        <li class="ep-m-0 ep-p-0">
                            <img src="<?php echo esc_url( $args->event->image_url );?>" alt="<?php echo esc_attr( $args->event->name );?>">
                        </li><?php
                    }
