...
<div id="ep-frontend-event-submission-section" class="emagic">
    ...
    if(isset($args->event) && isset($args->event->em_user_submitted)){
        if($args->event->em_user_submitted != get_current_user_id()){
            ?>
            <div class="ep-danger"><?php esc_html_e('Event does not exists.', 'eventprime-event-calendar-management');?></div>
            <?php
        }
    }
                    ...
                    <?php if(isset($args->fes_event_featured_image) && !empty($args->fes_event_featured_image)):?>
                            <div class="ep-form-row ep-box-row ep-mt-4">
                                ...
                                <?php esc_html_e('Featured Image', 'eventprime-event-calendar-management');?>
                                    ...
                                    <input type="file" name="featured_img" id="ep-fes-featured-file" onchange="upload_file_media(this)" accept="image/png, image/jpeg"/>
