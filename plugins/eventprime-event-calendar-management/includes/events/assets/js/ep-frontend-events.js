jQuery( function( $ ) {
    ...
    // cost range
    $( "#ep-cost-range" ).slider({
        range: true,
        min: 0,
        max: 500,
        values: [ 75, 300 ],
        slide: function( event, ui ) {
            $( "#ep-cost" ).val( "$" + ui.values[ 0 ] + " - $" + ui.values[ 1 ] );
        }
    });

    $( "#ep-cost" ).val( "$" + $( "#ep-cost-range" ).slider( "values", 0 ) + " - $" + $( "#ep-cost-range" ).slider( "values", 1 ) );
    ...
    $( document ).ready(function () {
        ...
        let column_header_format = eventprime.global_settings.calendar_column_header_format;
        if( !column_header_format || ( column_header_format !== 'ddd' && column_header_format !== 'dddd' ) ) {
            column_header_format = 'dddd';
        }
        // set day max events
        ...
        if( calendarEl ) {
            var calendar = new FullCalendar.Calendar( calendarEl, {
                ...
                titleFormat: eventprime.global_settings.calendar_title_format,
            ...
            if( calAreaWidht <= 720 ) {
                calendar.setOption( 'dayHeaderFormat', 'ddd' );
            }

            // custom class on the toolbal title
            let cal_title_class = 'ep-calendar-title-short';
            let calendar_title_format = eventprime.global_settings.calendar_title_format;
    ...
    $(document).on('click','#ep-loadmore-events',function(e){
        ...
        var max_page = $('#ep-loadmore-events').data('max');
        var paged = $('#ep-events-paged').val();
        var display_style = $('#ep-events-style').val();
        var limit = $('#ep-events-limit').val();
        var event_types_ids = $('#ep-events-types-ids').val();
        var event_venues_ids = $('#ep-events-venues-ids').val();
        var i_events = $('#ep-events-i-events').val();
        ...
        formData.append( 'event_atts', JSON.stringify( ep_frontend.event_attributes ) );
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#ep-events-paged').val(response.data.paged);
                if(response.data.paged >= max_page){
                    $('.ep-events-load-more').hide();
                }
    ...
    $( document ).on( 'change', '#ep-filter-types', function() {
        ...
        if( filter_value !== '') {
            // render html
            let param = { label: 'Event Types', key:'event_types', value: filter_value, text: filter_text };
            // update html
            event_applied_filters_render_content( param );
            //event_search_params.push( param );
            event_filters_selection_update(event_search_params, param);
    ...
    $( document ).on( 'change', '#ep-filter-venues', function() {
        ...
        if( filter_value !== '') {
            // render html
            let param = { label: 'Event Venue', key:'event_venues', value: filter_value, text: filter_text };
            // update html
            event_applied_filters_render_content( param );
            event_filters_selection_update(event_search_params, param);
            //let display_style = $('#ep-events-style').val();
    ...
    $( document ).on( 'change', '#ep-filter-performer', function() {
        ...
        if(filter_value.length == 0){
            ...
            // render html
            let param = { label: 'Performers', key:'event_performers', value: filter_value, text: filter_value.length+' Selected' };
            // update html
            event_applied_filters_render_content( param );
            event_filters_selection_update(event_search_params, param);
            //let display_style = $('#ep-events-style').val();
    ...
    $( document ).on( 'change', '#ep-filter-org', function() {
        ...
        if(filter_value.length == 0){
            ...
            // render html
            let param = { label: 'Organizers', key:'event_organizers', value: filter_value, text: filter_value.length+' Selected' };
            // update html
            event_applied_filters_render_content( param );
            event_filters_selection_update(event_search_params, param);
    ...
    // Event Card description Filter

    $(document).ready(function () {
        const selector = $('.ep-box-card-desc');
                ...
                if( response.success ) {
                    if( response.data.html ) {
                        ...
                        if( event_search_params && event_search_params.length > 0 ){
                            ...
                            jQuery( '#ep_total_filters_applied' ).html( event_search_params.length + ' ' + ep_frontend.filters_applied_text );
                        } else{
...
function ep_render_calendar_view( cal_events ) {
    ...
    if( calendarEl ) {
        ...
        if( calAreaWidht <= 720 ) {
            ...
            calendar.setOption( 'dayHeaderFormat', column_header_format );
        }
...
function epCard_width_adjust(cardClass) {
    ...
    if (kfWidth < 720) {
        $("#ep-events-content-container").addClass("narrow");
    }
    switch (true) {
        case kfWidth <= 500:
